...
var _extends = require("@babel/runtime/helpers/extends"), _objectWithoutPropertiesLoose = require("@babel/runtime/helpers/objectWithoutPropertiesLoose"), prettify = require("@emotion/css-prettifier");
...
var _extends__default = _interopDefault(_extends), _objectWithoutPropertiesLoose__default = _interopDefault(_objectWithoutPropertiesLoose), prettify__default = _interopDefault(prettify);
...
function getClassNamesFromEnzyme(selectors, node) {
  return getClassNames(selectors, getClassNameProp(findNodeWithClassName(shouldDive(node) ? node.dive() : node)));
}
...
function getClassNamesFromNodes(nodes) {
  return nodes.reduce((function(selectors, node) {
    return isEnzymeElement(node) ? getClassNamesFromEnzyme(selectors, node) : isCheerioElement(node) ? getClassNamesFromCheerio(selectors, node) : isReactElement(node) ? getClassNamesFromTestRenderer(selectors, node) : getClassNamesFromDOMElement(selectors, node);
  }), []);
}
...
function getStylesFromClassNames(classNames, elements) {
  ...
  var selectorPattern = new RegExp("\\.(?:" + filteredClassNames.map((function(cls) {
    return "(" + cls + ")";
  })).join("|") + ")"), rules = flatMap(elements, getElementRules), styles = rules.map((function(rule) {
    ...
    return _ref5[0];
  })).join(""), keyframesMap = getKeyframesMap(rules), keyframeNameKeys = Object.keys(keyframesMap), keyframesStyles = "";
  if (keyframeNameKeys.length) {
    ...
    styles = styles.replace(keyframesNamePattern, (function(name) {
      ...
      keyframesStyles += keyframesMap[name]), keyframesNameCache[name];
...
function getStyleElements() {
  if (!isBrowser) throw new Error("jest-emotion requires jsdom. See https://jestjs.io/docs/en/configuration#testenvironment-string for more information.");
...
function getMediaRules(rules, media) {
  return flatMap(rules.filter((function(rule) {
    return "@media" === rule.type && rule.value.replace(/\s/g, "").includes(media.replace(/\s/g, ""));
  })), (function(media) {
...
function deepTransform(node, transform) {
  if (Array.isArray(node)) return node.map((function(child) {
    return deepTransform(child, transform);
  }));
  var transformed = transform(node);
  return transformed !== node && transformed.children ? copyProps(transformed, {
...
function filterEmotionProps(props) {
  ...
  _props.__EMOTION_LABEL_PLEASE_DO_NOT_USE__, _objectWithoutPropertiesLoose__default.default(_props, [ "css", "__EMOTION_TYPE_PLEASE_DO_NOT_USE__", "__EMOTION_LABEL_PLEASE_DO_NOT_USE__" ]));
...
var createConvertEmotionElements = function(keys, printer) {
  return function(node) {
    ...
    if (isEmotionCssPropEnzymeElement(node)) {
      var expectedClassNames = flatMap((node.props.css.name || "").split(" "), (function(cssClassName) {
        return keys.map((function(key) {
          return key + "-" + cssClassName;
        }));
      }));
      if (isShallowEnzymeElement(node, expectedClassNames)) {
        var _className = [ node.props.className ].concat(expectedClassNames).filter(Boolean).join(" "), emotionType = node.props.__EMOTION_TYPE_PLEASE_DO_NOT_USE__, type = "string" == typeof emotionType ? emotionType : emotionType.name;
        return _extends__default.default({}, node, {
          props: filterEmotionProps(_extends__default.default({}, node.props, {
    ...
    return isEmotionCssPropElementType(node) ? _extends__default.default({}, node, {
      props: filterEmotionProps(node.props),
      type: node.props.__EMOTION_TYPE_PLEASE_DO_NOT_USE__
    }) : isReactElement(node) ? copyProps({}, node) : node;
  };
};

function clean(node, classNames) {
  if (Array.isArray(node)) {
    var _iterator3 = node, _isArray3 = Array.isArray(_iterator3), _i3 = 0;
    ...
    if (node.children) {
      var _iterator4 = node.children, _isArray4 = Array.isArray(_iterator4), _i4 = 0;
    ...
    if (node.props) {
      ...
      if (_className2) hasIntersection(_className2.split(" "), classNames) && delete node.props.css; else delete node.props.className;
    }
  }
}

function createSerializer(_temp) {
  var _ref6 = void 0 === _temp ? {} : _temp, classNameReplacer = _ref6.classNameReplacer, _ref6$DOMElements = _ref6.DOMElements, DOMElements = void 0 === _ref6$DOMElements || _ref6$DOMElements, cache = new WeakSet;
  return {
    test: function(val) {
      return val && !function(val) {
        return cache.has(val);
      }(val) && (isReactElement(val) || DOMElements && isDOMElement(val));
    },
    serialize: function(val, config, indentation, depth, refs, printer) {
      var elements = getStyleElements(), keys = getKeys(elements), converted = deepTransform(val, createConvertEmotionElements(keys)), nodes = getNodes(converted), classNames = getClassNamesFromNodes(nodes), styles = getPrettyStylesFromClassNames(classNames, elements, config.indent);
...
exports.createSerializer = createSerializer, exports.findLast = findLast, exports.getClassNamesFromNodes = getClassNamesFromNodes, 
exports.getMediaRules = getMediaRules, exports.getStyleElements = getStyleElements, 
exports.getStylesFromClassNames = getStylesFromClassNames, exports.hasClassNames = hasClassNames;
