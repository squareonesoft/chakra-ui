...
function filterEmotionProps(props) {
  ...
  var _props = props,
      ...
      rest = _objectWithoutPropertiesLoose__default['default'](_props, ["css", "__EMOTION_TYPE_PLEASE_DO_NOT_USE__", "__EMOTION_LABEL_PLEASE_DO_NOT_USE__"]);
...
var createConvertEmotionElements = function createConvertEmotionElements(keys, printer) {
  return function (node) {
    ...
    if (isEmotionCssPropEnzymeElement(node)) {
      ...
      if (isShallowEnzymeElement(node, expectedClassNames)) {
        var _className = [node.props.className].concat(expectedClassNames).filter(Boolean).join(' ');

        var emotionType = node.props.__EMOTION_TYPE_PLEASE_DO_NOT_USE__; // emotionType will be a string for DOM elements

        var type = typeof emotionType === 'string' ? emotionType : emotionType.name;
...
function clean(node, classNames) {
  if (Array.isArray(node)) {
    for (var _iterator3 = node, _isArray3 = Array.isArray(_iterator3), _i3 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator]();;) {
      var _ref4;
  ...
  if (node.children) {
    for (var _iterator4 = node.children, _isArray4 = Array.isArray(_iterator4), _i4 = 0, _iterator4 = _isArray4 ? _iterator4 : _iterator4[Symbol.iterator]();;) {
      var _ref5;
...
function createSerializer(_temp) {
  ...
  function serialize(val, config, indentation, depth, refs, printer) {
    ...
    var styles = getPrettyStylesFromClassNames(classNames, elements, config.indent);
