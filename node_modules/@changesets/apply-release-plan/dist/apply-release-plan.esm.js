...
async function getNewChangelogEntry(releasesWithPackage, changesets, config, cwd) {
  ...
  let moddedChangesets = await Promise.all(changesets.map(async cs => _objectSpread2(_objectSpread2({}, cs), {}, {
...
async function prependFile(filePath, data, name, prettierConfig) {
  ...
  if (!fileData) {
    ...
    await fs.writeFile(filePath, prettier.format(completelyNewChangelog, _objectSpread2(_objectSpread2({}, prettierConfig), {}, {
      filepath: filePath,
      parser: "markdown"
    })));
    return;
  ...
  await fs.writeFile(filePath, prettier.format(newChangelog, _objectSpread2(_objectSpread2({}, prettierConfig), {}, {
    filepath: filePath,
    parser: "markdown"
  })));
}
